...
class HSTProportionalControllerNode:
	...
	def scan_callback( self, msg ):
		...
		# print( "error: %.2f, slope: %.2f, intercept: %.2f" % (self.error_distance, self.line_fit[1], self.line_fit[0]) )
	...
	def controller( self ):
		...
		self.angle_toward_wall = -math.tan( self.line_fit[ 1 ] )		# current bearing in radians

		self.msg.drive.steering_angle = -self.kp * self.error_distance + self.kd * self.error_derivative
		if ( math.copysign( self.msg.drive.steering_angle, 1 ) > self.steering_saturation ):
		 	self.msg.drive.steering_angle = self.steering-saturation * math.copysign( 1, self.msg.drive.steering_angle )
	...
	def first_scan( self ):
		...
		# calculates useful indicies into the /scan range[] and sets values of class attributes
	...
	def calculate_error_distance( self ):
		# interprets /scan data to identify line of wall and distance from car to wall
		...
		self.line_fit = np.poly1d( self.line_fit_raw )		#  returns ( intercept, slope )
		# print( self.line_fit )
