...
class HSTImprovedPDControllerNode:
    def __init__( self ):
        ...
        self.debug_switch                   = True      # False turns off debug print statements


        # list ranges for various scan indicies (calculated by self.do_first_scan()
    ...
    def scan_callback( self, msg ):
        ...
        # determine error
        self.error_distance = self.calculate_left_error_distance()           #returns [left error, slope of left wall]
        self.left_error_distance = self.error_distance[0]
        self.left_error_slope = self.error_distance[1]
        self.temp_right_error = self.calculate_right_error_distance()   #returns [right_error, slope of right wall]
        self.right_error_distance = self.temp_right_error[0]
        self.right_error_slope = self.temp_right_error[1]
        if self.debug_switch:
            print( "left error: %.2f, slope: %.2f" % (self.left_error_distance, self.left_error_slope) )
            print( "right error: %.2f, slope: %.2f" % (self.right_error_distance. self.right_error_slope) )

        # determine first derivative of left error using velocity and wall angle
        self.left_error_derivative = self.approx_error_derivative( self.left_error_distance, "left" )
        self.right_error_derivative = self.approx_error_derivative( self.right_error_distance, "right" )
        if self.debug_switch:
    ...
    def controller( self ):
        ...
        self.msg.drive.steering_angle = self.kp * self.left_error_distance + self.kd * self.left_error_derivative
        if self.debug_switch:
            print("left error: %.2f, steering gain: %.2f, steering angle: %.2f" % ( self.left_error_distance, self.kp, self.msg.drive.steering_angle ) )
            print("left error derivative: %.2f, kd: %.2d" % ( self.left_error_derivative, self.kd ) )
        # cap steering angle at saturation value
        if ( abs( self.msg.drive.steering_angle ) > self.steering_saturation ):
            self.msg.drive.steering_angle = self.steering_saturation * math.copysign( 1, self.msg.drive.steering_angle )
        if self.debug_switch:
            print('left error distance: %.2f, derivative: %.2f steering_angle: %.2f' % ( self.left_error_distance, self.left_error_derivative, self.msg.drive.steering_angle ) )
    ...
    def first_scan( self ):
        ...
        self.scan_range_array_length = len( self.scan_msg.ranges )          # length of range list
        self.scan_range_array_center = self.scan_range_array_length // 2    # center of range list
        ...
        # determine indicies range list for desired agnles from center; set attribute values
    ...
    def calculate_left_error_distance( self ):
        ...
        if ( len( self.x_array ) > 10 ):
            ...
            self.shortest_distance = math.copysign( self.line_fit[0] / math.sqrt( 1 + self.line_fit[ 1 ] ** 2 ), 1 )
            if self.debug_switch:
    ...
    def calculate_right_error_distance( self ):
        ...
        self.right_x_array = []
        self.right_y_array = []
        for element in range( self.minus_60_range_index, self.minus_90_range_index ):
            # ignore distances beyond lidar range
            if ( self.scan_msg.ranges[ element ] < self.scan_msg.range_max ):
                self.result = self.calculate_cartesian_coords( element )
        ...
        # fit points to first order polynomial (line)
        if ( len( self.x_array ) > 10 ):
            self.right_line_fit_raw = np.polyfit( self.right_x_array, self.right_y_array, 1 )
            self.right_line_fit = np.poly1d( self.right_line_fit_raw )      #  [ intercept, slope ]
            if self.debug_switch:
                print( "slope: %.2d; intercept: %.2d" % ( self.right_line_fit[ 0 ], self.right_line_fit[ 1 ] ) )
            self.shortest_right_distance = math.copysign( self.right_line_fit[0] / math.sqrt( 1 + self.right_line_fit[ 1 ] ** 2 ), 1 )
            if self.debug_switch:
                print( "shortest distance: %.2f" % self.shortest_right_distance )
            return [self.set_point - self.shortest_right_distance, self.right_line_fit[1] ]     # [shortest distance, slope ]
        else:
            return [ 0, 0 ]
        
    def approx_error_derivative( self, error, side ):
        """maintains two lists of error distances and times.  (left and right) if list is too short ( <10) returns 0,
        otherwise calculates slope from list of errors and times"""
        if ( "left" in side ):
            self.left_error_list.append( error )
            t = rospy.Time( self.scan_msg.header.stamp.secs, self.scan_msg.header.stamp.nsecs )
            ...
            if len( self.left_error_list ) > 10:
                ...
                temp_derivative = np.polyfit( self.left_error_times, self.left_error_list, 1 )
            ...
            if len( self.right_error_list ) > 10:
                ...
                temp_derivative = np.polyfit( self.right_error_times, self.right_error_distances, 1 )
                return_val = ( np.poly1d( temp_derivative )[1] * 1000000000 )
            else:
                return_val = 0
        return return_val
