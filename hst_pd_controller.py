...
##
    def scan_callback( self, msg ):
        ...
        # determine error
        self.error_distance = self.calculate_error_distance()   #returns [error, slope of wall]
        if self.debug_switch:
                        print( "error: %.2f, slope: %.2f" % (self.error_distance[0], self.error_distance[1]) )

        # determine first derivative of error using velocity and wall angle
        self.error_derivative = self.approx_error_derivative( self.error_distance[0] )
        if self.debug_switch:
    ...
    def controller( self ):
        ...
        self.msg.drive.steering_angle = self.kp * self.error_distance[0] + self.kd * self.error_derivative
        if self.debug_switch:
            print("error: %.2f, steering gain: %.2f, steering angle: %.2f" % ( self.error_distance[0], self.kp, self.msg.drive.steering_angle ) )
            print("error derivative: %.2f, kd: %.2d" % ( self.error_derivative, self.kd ) )
        ...
        if self.debug_switch:
            print('error distance: %.2f, derivative: %.2f steering_angle: %.2f' % ( self.error_distance[0], self.error_derivative, self.msg.drive.steering_angle ) )
    ...
    def approx_error_derivative( self, error ):
        """maintains a lsist of error distances and times.  if list is too short ( <5) uses
        calculate_error_derivitave to return the derivative else calculate from list of values"""
